...
export default class CommandBuilder {
  ...
  async buildClaudeCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 180; // Claude needs up to 3 minutes for auth + full analysis
  ...
  async buildCodexCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 300; // Codex needs up to 5 minutes for full PR analysis
  ...
  async buildGeminiCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 120; // Gemini needs up to 2 minutes for full analysis
