...
export default class CommandBuilder {
  ...
  async buildClaudeCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 60; // Claude needs 60s for auth + analysis
  ...
  async buildCodexCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 30; // Codex needs 30s due to MCP startup
  ...
  async buildGeminiCommand(config, manifest, options) {
    ...
    const timeout = config.timeout_override || 30; // Gemini needs 30s for analysis
